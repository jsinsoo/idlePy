...
class QuizApp:
    ...
    def save_report(self, score, accuracy):
        filename = f"{self.student_id}_{self.student_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(filename, "w", encoding="utf-8") as f:
            f.write(f"학번: {self.student_id}\n이름: {self.student_name}\n점수: {score}/{len(self.quiz_objs)} ({accuracy:.1f}점)\n")
        print(f"성적표 저장 완료: {filename}")
